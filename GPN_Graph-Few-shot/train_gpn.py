...
parser.add_argument('--use_cuda', action='store_false', help='Enables CUDA training.')
...
parser.add_argument('--dataset', default='Amazon_clothing', help='Dataset:Amazon_clothing/Amazon_electronics/dblp/cora_full')
...
g, class_list_train, class_list_valid, class_list_test, id_by_class = load_data(dataset)

# Model and optimizer
encoder = GPN_Encoder(nfeat=g.x.shape[1],
            nhid=args.hidden,
            dropout=args.dropout)


scorer = GPN_Valuator(nfeat=g.x.shape[1],
            nhid=args.hidden,
            dropout=args.dropout)
...
def train(class_selected, id_support, id_query, n_way, k_shot):
    ...
    embeddings = encoder(g.x, g.edge_index)
    z_dim = embeddings.size()[1]
    scores = scorer(g.x, g.edge_index)
    ...
    # node importance
    support_degrees = torch.log(g.degree_in[id_support].view([n_way, k_shot]))
    ...
    labels_new = torch.LongTensor([class_selected.index(i) for i in g.y[id_query]])
    if args.cuda:
